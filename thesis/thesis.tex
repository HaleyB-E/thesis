...
btwn cycle 3 and 4:
rich14: bush's axis of evil speech triggers major jump in nuclear references
this, again, isn't part of a cycle because while it gets referenced as sign of hostility by DPRK it doesn't lead to immediate/direct fallout but rather is used as bonus justification for behavior in other contexts. that being said, this is sort of a conversation for the next section down.

Cycle 6:
rich14: references decline after the nuclear test
sin: while most incidents are framed in terms of "nationalism", this had rhetoric surrounding it that focused instead on "US imperialism"

Cycle 7:
rich14: references nearly double after nuclear test, but this could be due to other factors because w/regression testing, rhetoric draws down dramatically after testing
sin: where nationalism is the baseline framing for literally everything, it does not show up above average so maybe this isn't an ESPECIALLY nationalist missile strike (or maybe there were a lot of mixed incentives to do the thing - nationalist, US-focus, deterrence)  ALSO: major focus on "international" rhetoric vs. "US-focused" stuff - suggesting nuclear test was meant to be signal to everyone, not differentially the US

Cycle 8:
rich12: "In contrast, the two variables having the strongest negative relationship in substantive terms (coefficients) is not the country controls but rather references to Cheonan and Kim Jong Un, consistent with the assumption that Pyongyang would want to keep contemporary factors separate from general nuclear coverage"

Btwn 9 and 10:
kim jong il dies, rhetoric ramps up a lot. this is around when the leap day agreement between obama and dprk occurs, but isn't part of a cycle - no clear progression, though events likely impact one another in complex ways blah blah nothing is simple
...
\todo{finish dprk temporal rhetoric}
